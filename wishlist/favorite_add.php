...
$userID = $_SESSION['id'];
$listID = $_POST['wishID']; // Not sure where the IDs will come from: saved in SESSION or set by POST
//$listID = $_POST['listID'];
$productID = $_GET['productId']; // Not sure where the IDs will come from: saved in SESSION or set by POST
...
if (mysqli_num_rows($result) > 0) {
    echo "this item is already in this wishlist<a href=\"../wishlists.php\">Return to Wishlists</a>";
} else {
    echo var_dump($listID);
    echo "List ID is " . $listID . "<br>";
    echo "product ID is " . $productID;
    $add_favorite_query= "INSERT INTO favorites (listID,productID) VALUES ($listID, $productID)";
    echo $add_favorite_query;
